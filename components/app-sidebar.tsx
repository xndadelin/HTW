        ...
        <div>
          <SidebarHeader>
            <Link href="/" className="flex gap-2 items-center justify-center mt-4" prefetch={false}>
              ...
              <span className="text-lg font-bold text-neutral-800">Hack The Way</span>
            </Link>
          </SidebarHeader>
          <SidebarGroup>
                ...
                {items.map((item) => (
                  <SidebarMenuItem key={item.title}>
                    <SidebarMenuButton asChild>
                      <Link href={item.url} className="flex items-center gap-2 px-2 py-1 text-neutral-700 hover:bg-neutral-100 rounded transition-colors">
                        <item.icon size={18} />
            ...
            <SidebarMenuItem>
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <SidebarMenuButton>
                    <User2 />
                    <span className="ml-2">{user.user_metadata?.full_name || "Account"}</span>
